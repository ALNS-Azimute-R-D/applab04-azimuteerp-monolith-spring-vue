...
describe('Category e2e test', () => {
  ...
  const categorySample = { name: 'unhealthy despite' };
  ...
  describe('new Category page', () => {
    ...
    it('should create an instance of Category', () => {
      cy.get(`[data-cy="acronym"]`).type('closet');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'closet');

      cy.get(`[data-cy="name"]`).type('finally');
      cy.get(`[data-cy="name"]`).should('have.value', 'finally');

      cy.get(`[data-cy="description"]`).type('if thrifty yuck');
      cy.get(`[data-cy="description"]`).should('have.value', 'if thrifty yuck');

      cy.get(`[data-cy="handlerClazzName"]`).type('via exclamation inasmuch');
      cy.get(`[data-cy="handlerClazzName"]`).should('have.value', 'via exclamation inasmuch');
