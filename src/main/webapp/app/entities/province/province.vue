<template>
  <div>
    <h2 id="page-heading" data-cy="ProvinceHeading">
      <span v-text="t$('azimuteErpSpringVueMonolith04App.province.home.title')" id="province-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('azimuteErpSpringVueMonolith04App.province.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ProvinceCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('azimuteErpSpringVueMonolith04App.province.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && provinces && provinces.length === 0">
      <span v-text="t$('azimuteErpSpringVueMonolith04App.province.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="provinces">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('acronym')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.province.acronym')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'acronym'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.province.name')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('geoPolygonArea')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.province.geoPolygonArea')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('country.name')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.province.country')"></span>
        ...
        <tbody>
          <tr v-for="province in provinces" :key="province.id" data-cy="entityTable">
            ...
            <td>
              <a
                v-if="province.geoPolygonArea"
                v-on:click="openFile(province.geoPolygonAreaContentType, province.geoPolygonArea)"
                v-text="t$('entity.action.open')"
              ></a>
              <span v-if="province.geoPolygonArea">{{ province.geoPolygonAreaContentType }}, {{ byteSize(province.geoPolygonArea) }}</span>
            </td>
            <td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="azimuteErpSpringVueMonolith04App.province.delete.question"
          data-cy="provinceDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-province-heading" v-text="t$('azimuteErpSpringVueMonolith04App.province.delete.question', { id: removeId })"></p>
      </div>
