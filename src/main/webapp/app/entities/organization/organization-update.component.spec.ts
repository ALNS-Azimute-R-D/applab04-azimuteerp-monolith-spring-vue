/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import OrganizationUpdate from './organization-update.vue';
import OrganizationService from './organization.service';
import AlertService from '@/shared/alert/alert.service';

import TenantService from '@/entities/tenant/tenant.service';
import TypeOfOrganizationService from '@/entities/type-of-organization/type-of-organization.service';

type OrganizationUpdateComponentType = InstanceType<typeof OrganizationUpdate>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const organizationSample = { id: 123 };

describe('Component Tests', () => {
  let mountOptions: MountingOptions<OrganizationUpdateComponentType>['global'];
  let alertService: AlertService;

  describe('Organization Management Update Component', () => {
    let comp: OrganizationUpdateComponentType;
    let organizationServiceStub: SinonStubbedInstance<OrganizationService>;

    beforeEach(() => {
      route = {};
      organizationServiceStub = sinon.createStubInstance<OrganizationService>(OrganizationService);
      organizationServiceStub.retrieve.onFirstCall().resolves(Promise.resolve([]));

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'b-input-group': true,
          'b-input-group-prepend': true,
          'b-form-datepicker': true,
          'b-form-input': true,
        },
        provide: {
          alertService,
          organizationService: () => organizationServiceStub,
          tenantService: () =>
            sinon.createStubInstance<TenantService>(TenantService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
          typeOfOrganizationService: () =>
            sinon.createStubInstance<TypeOfOrganizationService>(TypeOfOrganizationService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
        },
      };
    });

    afterEach(() => {
      vitest.resetAllMocks();
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const wrapper = shallowMount(OrganizationUpdate, { global: mountOptions });
        comp = wrapper.vm;
        comp.organization = organizationSample;
        organizationServiceStub.update.resolves(organizationSample);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(organizationServiceStub.update.calledWith(organizationSample)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        organizationServiceStub.create.resolves(entity);
        const wrapper = shallowMount(OrganizationUpdate, { global: mountOptions });
        comp = wrapper.vm;
        comp.organization = entity;

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(organizationServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        organizationServiceStub.find.resolves(organizationSample);
        organizationServiceStub.retrieve.resolves([organizationSample]);

        // WHEN
        route = {
          params: {
            organizationId: '' + organizationSample.id,
          },
        };
        const wrapper = shallowMount(OrganizationUpdate, { global: mountOptions });
        comp = wrapper.vm;
        await comp.$nextTick();

        // THEN
        expect(comp.organization).toMatchObject(organizationSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        organizationServiceStub.find.resolves(organizationSample);
        const wrapper = shallowMount(OrganizationUpdate, { global: mountOptions });
        comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
