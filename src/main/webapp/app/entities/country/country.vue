<template>
  <div>
    <h2 id="page-heading" data-cy="CountryHeading">
      <span v-text="t$('azimuteErpSpringVueMonolith04App.country.home.title')" id="country-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('azimuteErpSpringVueMonolith04App.country.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'CountryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('azimuteErpSpringVueMonolith04App.country.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && countries && countries.length === 0">
      <span v-text="t$('azimuteErpSpringVueMonolith04App.country.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="countries">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('acronym')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.country.acronym')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'acronym'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.country.name')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('continent')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.country.continent')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'continent'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('geoPolygonArea')">
              <span v-text="t$('azimuteErpSpringVueMonolith04App.country.geoPolygonArea')"></span>
        ...
        <tbody>
          <tr v-for="country in countries" :key="country.id" data-cy="entityTable">
            ...
            <td>{{ country.name }}</td>
            <td v-text="t$('azimuteErpSpringVueMonolith04App.ContinentEnum.' + country.continent)"></td>
            <td>
              <a
                v-if="country.geoPolygonArea"
                v-on:click="openFile(country.geoPolygonAreaContentType, country.geoPolygonArea)"
                v-text="t$('entity.action.open')"
              ></a>
              <span v-if="country.geoPolygonArea">{{ country.geoPolygonAreaContentType }}, {{ byteSize(country.geoPolygonArea) }}</span>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="azimuteErpSpringVueMonolith04App.country.delete.question"
          data-cy="countryDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-country-heading" v-text="t$('azimuteErpSpringVueMonolith04App.country.delete.question', { id: removeId })"></p>
      </div>
